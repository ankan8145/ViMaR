...
from vlm_value_models_ad import ValueModel
...
if __name__ == "__main__":
    ...
    training_args.mixed_precision = "fp16"
    ...
    ckpt_path = "/vimar-ckpt-full-finetune_3/checkpoint-40500"
    value_net = ValueModel(model_pth)
    value_net.from_pretrained(ckpt_path)
    # value_net.to(device, dtype=torch.float16)
    # print(value_net.device)
    for param in value_net.llava_encoder.parameters():
    ...
    def collate_fn(examples):
        ...
        # batch = value_net.processor(text=current_state, images=images, padding=True, truncation=True, return_tensors="pt")
        # batch_next = value_net.processor(text=next_state, images=images, padding=True, truncation=True, return_tensors="pt")
    ...
    td_dataset_pth = 'output_files_1/train_data_with_clip_score_train.jsonl' # Your TD dataset path here
    td_dataset = load_value_dataset(td_dataset_pth)
    
    td_dataset_pth_test = 'output_files_1/train_data_with_clip_score_test.jsonl' 
    td_dataset_test = load_value_dataset(td_dataset_pth_test)
